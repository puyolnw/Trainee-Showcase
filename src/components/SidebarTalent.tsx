...
const items = [
  { to: "/app/talent", icon: Home, label: "Dashboard", end: true },
  { to: "/app/talent/reviews", icon: FileText, label: "My Reviews", end: false },
  { to: "/app/talent/goals", icon: Target, label: "Goal Setting", end: false },
  { to: "/app/talent/self-assessment", icon: UserCheck, label: "Self-Assessment", end: false },
  { to: "/app/talent/feedback", icon: Users, label: "360 Feedback", end: false },
  { to: "/app/talent/check-ins", icon: MessageSquare, label: "Check-ins", end: false },
  { to: "/app/talent/idp", icon: BookOpen, label: "IDP", end: false },
];

interface SidebarTalentProps {
  onClose?: () => void;
  isMobile?: boolean;
}

export default function SidebarTalent({ onClose, isMobile = false }: SidebarTalentProps) {
  // Load sidebar state from localStorage immediately on mount
  // Use shared key for all layouts
        ...
        <div className="mb-6 w-full flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 flex-1 p-2 rounded-lg">
            <div className="w-10 h-10 bg-gradient-to-br from-ptt-blue to-ptt-cyan rounded-xl flex items-center justify-center shadow-lg shadow-ptt-blue/20">
              <Target className="w-6 h-6 text-app" strokeWidth={2} />
            </div>
            <div className="flex-1 text-left">
              <h2 className="text-lg font-semibold text-[var(--accent)] font-display">HREx Talent</h2>
              <p className="text-xs text-muted font-light">Performance Management</p>
            </div>
          </div>
        </div>
